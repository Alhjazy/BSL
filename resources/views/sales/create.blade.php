...
@section('content')
    ...
    <div class="intro-y box p-5 mt-5">
        <div class="flex flex-col sm:flex-row sm:items-end xl:items-start">
            <form class="xl:flex sm:mr-auto" id="order_info">
                <div class="sm:flex items-center sm:mr-4">
                    <div class="flex">
                        ...
                        <select name="branch_id" id="branch_id" class="tail-select branch_id w-40" >
                            @if($branches) @foreach($branches as $key => $value)
                ...
                <div class="sm:flex items-center sm:mr-4 mt-2 xl:mt-0">
                    <div class="flex">
                        <div class="z-30 rounded-l w-20  flex items-center justify-center bg-gray-100 border text-gray-600 dark:bg-dark-1 dark:border-dark-4 -mr-1">Customer</div>
                        <select name="customer_id" id="customer_id" class="tail-select customer_id w-40" >
                            @if($customers) @foreach($customers as $key => $value)
                                <option value="{{$value->id}}">{{$value->name}}</option>
                            @endforeach @endif
                        </select>
                        <a  href="javascript:;" data-toggle="modal" data-target="#customer-modal-preview" class="z-30 rounded-r w-10 flex items-center justify-center bg-theme-1 border text-white dark:bg-dark-1 dark:border-dark-4 -ml-1">+</a>
                    </div>
                </div>
                <br>
                <hr>
    ...
    <div class="modal" id="customer-modal-preview">
        <div  class="modal__content modal__content--xl p-10">
            <div class="flex items-center px-5 py-5 sm:py-3 border-b border-gray-200 dark:border-dark-5">
                <h2 class="font-medium text-base mr-auto">Registering Customer</h2>
                <button class="button border items-center text-gray-700 dark:border-dark-5 dark:text-gray-300 hidden sm:flex"> <i data-feather="file" class="w-4 h-4 mr-2"></i> Download Docs </button>
                <div class="dropdown sm:hidden"> <a class="dropdown-toggle w-5 h-5 block" href="javascript:;"> <i data-feather="more-horizontal" class="w-5 h-5 text-gray-600 dark:text-gray-600"></i> </a>
                    <div class="dropdown-box w-40">
                        <div class="dropdown-box__content box dark:bg-dark-1 p-2"> <a href="javascript:;" class="flex items-center p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md"> <i data-feather="file" class="w-4 h-4 mr-2"></i> Download Docs </a> </div>
            ...
            <form action="#" method="post" id="supplier_inputs" onsubmit="addCustomer(this,event);" >
                @csrf
                <div class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                    <div class="col-span-12 sm:col-span-6"> <label>Customer Name</label> <input name="name"  type="text" class="input w-full border mt-2 flex-1" > </div>
                    <div class="col-span-12 sm:col-span-6"> <label>Customer Phone Number</label> <input name="phone_number"  type="text" class=" input w-full border mt-2 flex-1" > </div>
                    <div class="col-span-12 sm:col-span-6"> <label>Customer E-mail</label> <input name="email" type="text" class=" input w-full border mt-2 flex-1" > </div>
                    <div class="col-span-12 sm:col-span-6"> <label>Customer Tax ID</label> <input name="tax_id"  type="text" class="input w-full border mt-2 flex-1" > </div>
                </div>
        ...
        function addCustomer(el,e) {
            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    form[0].reset();
                    cash('#customer-modal-preview').modal('hide');
                }
            });
            var newOption = new Option(customer_name, customer_id, true, true);
...
//                $('select.supplier_id').val(supplier_id).trigger('change');
        ...
        function save() {
            ...
            if(info.branch_id === ''){
                alert('Please Branch for this order!');
                return false;
            }
            var total_due = 0;
            $.each(payments,function (k,v) {
